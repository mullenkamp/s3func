...
def add_metadata_from_urllib3(response):
    ...
    for key, value in headers.items():
        ...
        if key == 'content-length':
            ...
            if '_u' in value:
                metadata['upload_timestamp'] = datetime.datetime.fromtimestamp(
                    int(value.split('_u')[1]) * 0.001, datetime.timezone.utc
                )
...
def add_metadata_from_s3_xml(response, method=None):
    ...
    if (response.status // 100) != 2:
        try:
            ...
            return metadata, {
                'Code': 'ParseError',
                'Message': 'Could not parse XML error response: ' + str(response.data),
            }
...
def get_metadata_from_b2_put_object(response):
    ...
    if 'upload_timestamp' in meta:
        meta['upload_timestamp'] = datetime.datetime.fromtimestamp(
            meta['upload_timestamp'] * 0.001, datetime.timezone.utc
        )
